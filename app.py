...
class Fighter():
    def __init__(self, x, y, name ,max_hp, strength, potions) :
        ...
        self.animation_list = []
        self.frame_index = 0
        self.action = 0 # 0:idle ,1:attack ,2:hurt ,3:head
        self.update_time = pygame.time.get_ticks()
        #load Idle images
        temp_list = [] 
        for i in range(8):
            img = pygame.image.load(f'{self.name}/Idle/{i}.png').convert_alpha()
        ...
        temp_list = [] 
        for i in range(8):
            img = pygame.image.load(f'{self.name}/Attack/{i}.png').convert_alpha()
            img = pygame.transform.scale(img,(img.get_width()*3,img.get_height()*3))
            temp_list.append(img)
        self.animation_list.append(temp_list)
        #load  Hurt images
        temp_list = [] 
        for i in range(3):
            img = pygame.image.load(f'{self.name}/Hurt/{i}.png').convert_alpha()
            img = pygame.transform.scale(img,(img.get_width()*3,img.get_height()*3))
            temp_list.append(img)
        self.animation_list.append(temp_list)
        #load  Death images
        temp_list = [] 
        for i in range(10):
            img = pygame.image.load(f'{self.name}/Death/{i}.png').convert_alpha()
            img = pygame.transform.scale(img,(img.get_width()*3,img.get_height()*3))
            temp_list.append(img)
        self.animation_list.append(temp_list)
    ...
    def update(self):
        ...
        # current time - time last updated > 100 ml seconds?
        if pygame.time.get_ticks() - self.update_time > animation_cooldown:
            self.update_time = pygame.time.get_ticks()
            self.frame_index +=1
            
        # if animation ran out: reset to start
        ...
        for count, bandit in enumerate(bandit_list):
            if current_fighter == 2 + count:
                if bandit.alive == True:
                    action_cooldown += 1
                    if action_cooldown >= action_wait_time:
                        #check if bandit heals before attack
